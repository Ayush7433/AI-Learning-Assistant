...
export const chunkText = (text, chunkSize = 500, overlap = 50) => {
  ...
  for (const paragraph of paraghraphs) {
    ...
    const paraghraphWordCount = paragraphWords.length;

    // If single paragraph exceeds chunk size, split it by words
    if (paraghraphWordCount > chunkSize) {
    ...
    if (
      currentWordCount + paraghraphWordCount > chunkSize &&
      ...
      currentWordCount += paraghraphWordCount;
    }
  }
...
export const findRelevantChunks = (chunks, query, maxChunks = 3) => {
  ...
  const queryWords = query
    ...
    const scoredChunks = chunks.map((chunk, index) => {
        ...
        //return clean objech without Mongoose metadata
        return {
            content: chunk.content,
            chunkIndex: chunk.chunkIndex,
            pagenumber: chunk.pagenumber,
